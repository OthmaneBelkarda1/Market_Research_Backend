...
def premiere_phrase(texte: str, max_mots: int) -> str:
    ...
    Args:
        texte: Texte source.
        max_mots: Plafond de mots.

    Returns:
        La première phrase, coupée au mot entier si elle dépasse le plafond.
    """
    propre = " ".join((texte or "").split())
    for separateur in (". ", " POURQUOI", " ; "):
        position = propre.find(separateur)
        if 0 < position:
...
def construire_ligne_sources(entrees: Any) -> str:
    ...
    Returns:
        La ligne « Sources analysées : … », ou une chaîne vide.
    """
    morceaux: list[str] = []
    ...
    for source in ("amazon", "aliexpress"):
        details = []
        if offres.get(source):
            details.append(f"{formater_nombre(offres[source], 0)} offres")
        if avis.get(source):
            details.append(f"{formater_nombre(avis[source], 0)} avis")
        if details:
            morceaux.append(f"{LIBELLES_SOURCES[source]} ({', '.join(details)})")

    if avis.get("reddit"):
        morceaux.append(
            f"Reddit ({formater_nombre(avis['reddit'], 0)} contributions)"
        )
    if stats and stats.nb_pages:
        morceaux.append(f"Web ({formater_nombre(stats.nb_pages, 0)} pages)")
    if stats is not None:
        morceaux.append(
            f"Publicité ({formater_nombre(stats.nb_annonces, 0)} annonces)"
        )

    recommandations = entrees.recommandations
    dossier = recommandations.dossier_synthese if recommandations else None
    tendances = dossier is not None and dossier.demande is not None
    morceaux.append(f"Tendances ({'disponible' if tendances else 'non collectée'})")

    return f"Sources analysées : {' · '.join(morceaux)}." if morceaux else ""
...
def retirer_ellipses(injectables: Injectables) -> int:
    ...
    La troncature de la version 1 coupait déjà au dernier mot entier avant
    d'ajouter « … » : retirer le caractère suffit donc à obtenir la coupe nette
    voulue, sans perdre un seul mot. Ce qui disparaît est le signe d'une coupe,
    pas son contenu — et c'est bien le signe qui trompait le lecteur, en laissant
    croire qu'une phrase se poursuivait alors qu'elle était perdue.
    ...
    injectables.badges = {
        cle: couper_a_la_phrase(valeur) if "…" in valeur else valeur
        for cle, valeur in injectables.badges.items()
    }
...
def couper_a_la_phrase(texte: str) -> str:
    """Ramène un texte à sa dernière phrase complète, sans ellipse.

    Args:
        texte: Texte éventuellement tronqué, ellipse comprise.

    Returns:
        Le texte jusqu'à la dernière ponctuation forte ; à défaut de ponctuation,
        le texte sans son ellipse.
    """
    propre = texte.replace("…", "").rstrip()
...
def enrichir(
    ...
    manques.extend(
        premiere_phrase(manque, MAX_MOTS_PUCE)
    ...
    injectables.ligne_sources = construire_ligne_sources(entrees)
    blocs_manquants = [
    ...
    if blocs_manquants:
        injectables.encart_partielle_v2 = ENCART_ETUDE_PARTIELLE_V2.format(
            blocs=", ".join(blocs_manquants),
            accord="non disponibles"
            if len(blocs_manquants) > 1
            else "non disponible",
        )
    ...
    injectables.divergences = [
        premiere_phrase(divergence, MAX_MOTS_PUCE)
    ...
    if dossier is not None and dossier.demande is not None:
        injectables.dynamique_demande = injectables.tableau_demande
    else:
    ...
    injectables.puces_personne_ne_fait = [
        premiere_phrase(angle, MAX_MOTS_PUCE_CONCURRENTS)
    ...
    if injectables.phase_brute and entrees.plc is not None:
        ...
        injectables.puces_phase = [
            couper_mots(r.enonce, MAX_MOTS_ACTION)
    ...
    # Le libellé amont est parfois une phrase entière : il est borné lui aussi,
    # sinon la puce dépasse son budget avant même sa condition de capture.
    injectables.puces_opportunites = [
        f"**{premiere_phrase(o.libelle, MAX_MOTS_CELLULE_COURTE)}** — "
        f"{premiere_phrase(o.conditions_de_capture[0] if o.conditions_de_capture else o.description, MAX_MOTS_PUCE - MAX_MOTS_CELLULE_COURTE)}"
        for o in recommandations.opportunites[:NB_OPPORTUNITES]
    ]
    injectables.puces_risques = [
        f"**{premiere_phrase(r.libelle, MAX_MOTS_CELLULE_COURTE)}** — "
        f"{premiere_phrase(r.attenuation, MAX_MOTS_PUCE - MAX_MOTS_CELLULE_COURTE)}"
    ...
    reste = "\n\n".join(
        ...
        for partie in (
            ...
            "\n".join(
                f"- **{o.libelle}** — {couper_mots(o.description, 30)}"
                for o in recommandations.opportunites[NB_OPPORTUNITES:]
            ),
            "**Autres risques**"
            ...
            "\n".join(
                f"- **{r.libelle}** ({r.gravite}) — {couper_mots(r.attenuation, 30)}"
                for r in recommandations.risques[NB_RISQUES:]
            ),
        )
        if partie
